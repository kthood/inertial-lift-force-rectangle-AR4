...
W = 180e-6; H = 45e-6;       % height and width of channel (m)
...
for j=1:size(xs,1)
    for l=1:size(xs,2)
        [forcex(j,l),forcey(j,l)]=getLiftForce_AR4(xs(j,l),ys(j,l),alpha,U,rho,L);
    end
end
...
% Advect particles using the lift force
% fast method
load inertial_constants_AR4_n401_05-12-2017.mat
x=zeros(m,N); y=zeros(m,N);
u =zeros(m,1); v=zeros(m,1);
x(:,1)=r(:,1); y(:,1)=r(:,2); 
for j=2:N;
...
figure('Units','pixels','Position',[100,100,800,275])
clf
