% liftforce_AR4_channel_plot

Re=36; L=45*10^-6; AR=4; rsph=6*10^-6; 
...
for j=1:size(xs,1)
    for l=1:size(xs,2)
        [forcex(j,l),forcey(j,l)]=getLiftForce_AR4(xs(j,l),ys(j,l),rsph,U,rho,L);
    end
end

figure('Units', 'pixels', 'Position', [100 400 800 200]);
h=10^6;
hold on
